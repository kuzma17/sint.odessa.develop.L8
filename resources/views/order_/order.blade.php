    ...
    <div class="content-page">
        ...
        <form name="order_order" method="post" class="form-horizontal" action="{{ url('/order') }}">
            ...
            <div class="form-group{{ $errors->has('order_client_name') ? ' has-error' : '' }}">
                ...
                <div class="col-md-9">
                    <input placeholder="Фамилия Имя Отчество" type="text" class="form-control" name="order_client_name" value="{{ old('order_client_name', isset($order)? $order->order_client_name: '') }}" @if(isset($user->profile->client_name)) readonly @endif autofocus>
            ...
            <div class="form-group client_company_order{{ $errors->has('order_user_company') ? ' has-error' : '' }}" @if((old() && old('order_type_client') == 1) || (!old() && isset($order) && $order->order_type_client == 1 )) style="display: none" @endif>
                ...
                <div class="col-md-9">
                    <input placeholder="Фамилия Имя Отчество контактного лица компании." id="user_company" type="text" class="form-control" name="order_user_company" value="{{ old('order_user_company', isset($order)? $order->order_user_company: '') }}" >
            ...
            <div class="form-group{{ $errors->has('order_phone') ? ' has-error' : '' }}">
                ...
                <div class="col-md-9">
                    <input  type="text" class="form-control" name="order_phone" value="{{ old('order_phone', isset($order)? $order->order_phone: '') }}" placeholder="номер мобильного телефона (050xxxxxxx)">
            ...
            <div class="form-group{{ $errors->has('order_delivery_town') || $errors->has('order_delivery_street') || $errors->has('order_delivery_house') ? ' has-error' : '' }}">
                ...
                <div class="col-md-9">
                    <div class="col-md-6" style=" padding:5px">
                        <input type="text" class="form-control" name="order_delivery_town" value="{{ old('order_delivery_town', isset($order)? $order->order_delivery_town: '') }}" placeholder="город, населенный пункт">
                    </div>
                ...
                <div class="col-md-9">
                    <div class="col-md-6" style=" padding:5px">
                        <input type="text" class="form-control" name="order_delivery_street" value="{{ old('order_delivery_street', isset($order)? $order->order_delivery_street: '') }}" placeholder="улица">
                        <label class="control-label label1">улица<span class="red">*</span></label>
                    </div>
                    <div class="col-md-2" style=" padding: 5px">
                        <input type="text" class="form-control" name="order_delivery_house" value="{{ old('order_delivery_house', isset($order)? $order->order_delivery_house: '') }}" placeholder="номер">
                        <label class="control-label label1">дом<span class="red">*</span></label>
                    </div>
                    <div class="col-md-2" style=" padding: 5px">
                        <input type="text" class="form-no-control" name="order_delivery_house_block" value="{{ old('order_delivery_house_block', isset($order)? $order->order_delivery_house_block: '') }}" placeholder="корпус">
                        <label class="label1">корпус</label>
                    </div>
                    <div class="col-md-2" style=" padding: 5px">
                        <input type="text" class="form-no-control" name="order_delivery_office" value="{{ old('order_delivery_office', isset($order)? $order->order_delivery_office: '') }}" placeholder="квартира">
                ...
                <div class="col-md-9">
                    <select name="service_office" class="form-control" autofocus>
                        @foreach($offices as $office)
                            <option value="{{ $office->id }}" @if((old() && old('service_office') == $office->id) || (!old() && isset($order) && $order->service_office == $office->id)) selected @endif>{{ $office->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="form-group{{ $errors->has('order_comment') ? ' has-error' : '' }}">
                ...
                <div class="col-md-9">
                    <textarea class="form-control" name="order_comment" placeholder="Например, укажите количество картриджей или описание неисправности техники).">{{ old('order_comment', isset($order)? $order->order_comment: '') }}</textarea>
